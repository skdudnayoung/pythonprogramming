...
def updatePhotoAndLabel():
    global num
    photo = PhotoImage(file=base_path + fnameList[num])
    pLabel.configure(image=photo)
    pLabel.image = photo
    fileNameLabel.config(text=fnameList[num])  # 파일명 업데이트
...
# 파일명 표시용 라벨 (버튼 사이)
fileNameLabel = Label(window, text=fnameList[num], font=("Arial", 12))
fileNameLabel.pack(side=LEFT, padx=10)

# 다음 버튼
btnNext = Button(window, text="다음 >>", command=clickNext)
btnNext.pack(side=LEFT, padx=10, pady=10)
